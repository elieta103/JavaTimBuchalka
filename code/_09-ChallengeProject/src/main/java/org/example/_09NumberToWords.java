...
public class _09NumberToWords {
    ...
    public static void numberToWords(int n1){
        if (n1 < 0){
        ...
        int reverse = reverse(n1);
        int zerosToWrite = getDigitCount(n1) -getDigitCount(reverse);
        while(reverse > 0){
    ...
    public static int getDigitCount(int n1) {
        if( n1<0 ){
            return -1;
        }
        if(n1==0){
